...
class TestModConfig(unittest.TestCase):
    ...
    def test_register_food_item(self):
        ...
        food_item = FoodItem(
            id="testmod:test_food", name="Test Food", nutrition=4, saturation=0.3
        )
    ...
    def test_to_java_constant_simple(self):
        ...
        # ID with namespace
        self.assertEqual(
            mod_config._to_java_constant("testmod:example_item"), "TESTMOD_EXAMPLE_ITEM"
        )
    ...
    def test_to_java_constant_special_chars(self):
        ...
        self.assertEqual(
            mod_config._to_java_constant("test-mod:my-item"), "TEST_MOD_MY_ITEM"
        )
    ...
    def test_custom_groups_property_with_custom_groups(self):
        ...
        item3 = Item(
            id="testmod:item3", name="Item 3", item_group=custom_group1
        )  # Same group
        mod_config.registerItem(item1)
        mod_config.registerItem(item2)
    ...
    def test_clone_repository(self, mock_subprocess):
        ...
        mod_config.clone_repository(
            "https://github.com/test/repo.git", self.project_dir
        )
    ...
    def test_update_mod_metadata(self):
        ...
        # Update metadata
        mod_config.update_mod_metadata(
            fabric_json_path,
            {
                "id": "testmod",
                "name": "Test Mod",
    ...
    def test_write_recipe_files_no_recipes(self):
        ...
        recipe_dir = os.path.join(
            self.project_dir, "src", "main", "resources", "data", "testmod", "recipe"
        )
    ...
    def test_write_recipe_files_with_recipes(self):
        ...
        item = Item(id="testmod:stone_item", name="Stone Item", recipe=recipe)
        mod_config.registerItem(item)
    ...
    @patch("fabricpy.modconfig.os.chdir")
    @patch("fabricpy.modconfig.os.getcwd")
    def test_run_method_with_gradle_properties(
        self, mock_getcwd, mock_chdir, mock_subprocess
    ):
    ...
    @patch("fabricpy.modconfig.os.chdir")
    @patch("fabricpy.modconfig.os.getcwd")
    def test_run_method_with_special_characters_in_mod_id(
        self, mock_getcwd, mock_chdir, mock_subprocess
    ):
...
"""
        ...
        self.assertIn(
            "# Done to increase the memory available to gradle.", final_content
        )
        ...
        self.assertIn("minecraft_version=1.21.11", final_content)
        self.assertIn("loader_version=0.18.4", final_content)
        self.assertIn("loom_version=1.15-SNAPSHOT", final_content)
        ...
        self.assertIn("fabric_version=0.141.3+1.21.11", final_content)
...
class TestModConfigIntegration(unittest.TestCase):
    ...
    def test_tool_item_properties_applied(self):
        ...
        self.assertIn("Attributes.ATTACK_DAMAGE", custom_content)
        self.assertIn("Operation.ADD_VALUE", custom_content)
        self.assertIn("EquipmentSlotGroup.MAINHAND", custom_content)
    ...
    def test_multiple_items_same_group(self):
        ...
        bow = Item(id="grouped:test_bow", name="Test Bow", item_group=weapons_group)

        shield = Item(
            id="grouped:test_shield", name="Test Shield", item_group=weapons_group
        )
    ...
    def test_mixed_vanilla_and_custom_groups(self):
        ...
        vanilla_item1 = Item(id="mixed:tool", name="Tool", item_group=item_group.TOOLS)

        vanilla_item2 = Item(
            id="mixed:food", name="Food", item_group=item_group.FOOD_AND_DRINK
        )

        # Items with custom group
        custom_item1 = Item(
            id="mixed:special1", name="Special Item 1", item_group=custom_group
        )

        custom_item2 = Item(
            id="mixed:special2", name="Special Item 2", item_group=custom_group
        )
