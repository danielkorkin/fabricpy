...
class TestFabricCompilationWorkflow(unittest.TestCase):
    ...
    def _create_test_components(self, mod: ModConfig):
        ...
        custom_group = ItemGroup(
            id="fabric_workflow_test:workflow_group", name="Workflow Test Group"
        )
            ...
            recipe=RecipeJson(
                {
                    ...
                    "ingredients": [
                        "minecraft:apple",
                        "minecraft:sugar",
                        "fabric_workflow_test:workflow_item",
                    ],
    ...
    def _verify_fabric_testing_setup(self, project_dir: str):
        ...
        test_dirs = ["src/test/java", "src/gametest/java", "src/gametest/resources"]

        for test_dir in test_dirs:
            full_path = os.path.join(project_dir, test_dir)
            self.assertTrue(
                os.path.exists(full_path), f"Test directory should exist: {test_dir}"
            )
        ...
        self.assertTrue(
            os.path.exists(gametest_fabric_mod), "Gametest fabric.mod.json should exist"
        )
    ...
    def _verify_unit_tests_generated(self, project_dir: str, mod: ModConfig):
        ...
        for test_file in expected_test_files:
            ...
            self.assertTrue(
                os.path.exists(test_path), f"Unit test should exist: {test_file}"
            )
    ...
    def _verify_game_tests_generated(self, project_dir: str, mod: ModConfig):
        ...
        for test_file in expected_files:
            ...
            self.assertTrue(
                os.path.exists(test_path), f"Game test should exist: {test_file}"
            )
    ...
    def _verify_server_gametest_content(self, gametest_dir: str, mod: ModConfig):
        ...
        if os.path.exists(server_test_path):
            with open(server_test_path, "r") as f:
                ...
                self.assertIn(
                    "FabricGameTest", content, "Should implement FabricGameTest"
                )
                self.assertIn("@GameTest", content, "Should use @GameTest annotation")
                self.assertIn(
                    "GameTestHelper", content, "Should use GameTestHelper parameter"
                )
                self.assertIn(
                    "EMPTY_STRUCTURE", content, "Should use EMPTY_STRUCTURE template"
                )
                self.assertIn(
                    "context.complete()", content, "Should call context.complete()"
                )
    ...
    def test_fabric_testing_gradle_tasks(self):
        ...
        test_item = Item(id="gradle_task_test:test_item", name="Test Item")
    ...
    def test_comprehensive_testing_features(self):
        ...
        self.assertTrue(
            os.path.exists(mod.project_dir), "Project directory should exist"
        )

        # Verify test files contain edge case handling
        test_dir = os.path.join(
            mod.project_dir, "src/test/java/com/example", mod.mod_id, "test"
        )
        ...
        if os.path.exists(test_dir):
            ...
            if os.path.exists(item_test_path):
                with open(item_test_path, "r") as f:
                    ...
                    self.assertIn(
                        "edge_case_food", content, "Should test edge case food item"
                    )
                    self.assertIn(
                        "complex_item", content, "Should test complex recipe item"
                    )
    ...
    def test_fabric_testing_documentation_compliance(self):
        ...
        # Add test item
        test_item = Item(id="docs_compliance_test:docs_item", name="Docs Test Item")
        mod.registerItem(test_item)
        mod.compile()
    ...
    def _verify_docs_compliance(self, project_dir: str):
        ...
        for test_file in unit_tests:
            with open(test_file, "r") as f:
                ...
                self.assertIn("import net.minecraft.server.Bootstrap;", content)
