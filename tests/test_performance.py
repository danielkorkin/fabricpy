...
class TestPerformance(unittest.TestCase):
    ...
    def test_create_many_items(self):
        ...
        self.assertLess(
            creation_time, 1.0, f"Creating 1000 items took {creation_time:.3f}s"
        )
    ...
    def test_create_many_food_items(self):
        ...
        self.assertLess(
            creation_time, 0.5, f"Creating 500 food items took {creation_time:.3f}s"
        )
    ...
    def test_create_many_blocks(self):
        ...
        for i in range(750):
            block = Block(
                ...
                inventory_texture_path=f"textures/items/block_{i}.png"
                if i % 2 == 0
                else None,
        ...
        self.assertLess(
            creation_time, 0.75, f"Creating 750 blocks took {creation_time:.3f}s"
        )
        self.assertEqual(len(blocks), 750)

        # Verify texture fallback works
        blocks_with_fallback = [
            b for b in blocks if b.inventory_texture_path == b.block_texture_path
        ]
    ...
    def test_create_many_recipes(self):
                ...
                recipe = RecipeJson(
                    {
                        ...
                        "ingredients": [
                            f"test:ingredient_{i}",
                            f"test:ingredient_{i + 1}",
                        ],
        ...
        self.assertLess(
            creation_time, 0.5, f"Creating 300 recipes took {creation_time:.3f}s"
        )
    ...
    def test_large_mod_registration(self):
        ...
        self.assertLess(
            registration_time,
            1.0,
            f"Registering 450 items/blocks took {registration_time:.3f}s",
        )
    ...
    def test_recipe_parsing_performance(self):
        ...
        self.assertLess(
            parsing_time, 0.5, f"Parsing 100 complex recipes took {parsing_time:.3f}s"
        )
    ...
    def test_mixed_component_creation(self):
        ...
        for i in range(1000):
            if i % 4 == 0:
                ...
                item = Item(
                    id=f"stress_test:crafted_{i}", name=f"Crafted {i}", recipe=recipe
                )
                mod.registerItem(item)
    ...
    def test_complex_recipe_stress(self):
        ...
        for i in range(500):
            recipe = RecipeJson(
                {
                    "type": "minecraft:crafting_shaped",
                    "pattern": [f"A{i % 10}B", f"{i % 10}C{i % 10}", f"D{i % 10}E"],
                    "key": {
