...
class TestFabricPyIntegration(unittest.TestCase):
        ...
        advanced_recipe = RecipeJson(
            {
                ...
                "result": {"id": "integration_test:advanced_tool", "count": 1},
            }
        )
        ...
        food_recipe = RecipeJson(
            {
                ...
                "result": {"id": "integration_test:super_food", "count": 2},
            }
        )
        ...
        block_recipe = RecipeJson(
            {
                "type": "minecraft:crafting_shaped",
                "pattern": ["###", "###", "###"],
                "key": {"#": "integration_test:rare_crystal"},
                "result": {"id": "integration_test:crystal_block", "count": 1},
            }
        )
    ...
    def test_recipe_system_integration(self):
        ...
        precious_item = Item(
            id="recipe_test:precious_ingot", name="Precious Ingot", recipe=shaped_recipe
        )
        ...
        refined_item = Item(
            id="recipe_test:refined_ingot", name="Refined Ingot", recipe=smelting_recipe
        )
        ...
        refined_block = Block(
            id="recipe_test:refined_block", name="Refined Block", recipe=block_recipe
        )
        ...
        items_with_recipes = [
            i for i in mod_config.registered_items if getattr(i, "recipe", None)
        ]
        blocks_with_recipes = [
            b for b in mod_config.registered_blocks if getattr(b, "recipe", None)
        ]
    ...
    def test_item_group_system_integration(self):
        ...
        # Armor group
        helmet = Item(
            id="group_test:helmet", name="Test Helmet", item_group=armor_group
        )
        chestplate = Item(
            id="group_test:chestplate", name="Test Chestplate", item_group=armor_group
        )
        ...
        # Vanilla groups
        vanilla_item1 = Item(
            id="group_test:ingredient",
            name="Ingredient",
            item_group=item_group.INGREDIENTS,
        )
        vanilla_item2 = Item(
            id="group_test:food", name="Food", item_group=item_group.FOOD_AND_DRINK
        )

        # Blocks with custom groups
        weapon_block = Block(
            id="group_test:weapon_rack", name="Weapon Rack", item_group=weapons_group
        )
        utility_block = Block(
            id="group_test:utility_bench",
            name="Utility Bench",
            item_group=utilities_group,
        )
    ...
    def test_edge_cases_and_error_handling(self):
        ...
        max_item = Item(id="edge_test:max_item", name="Maximum Item", max_stack_size=64)
        ...
        self.assertEqual(
            fallback_block.inventory_texture_path, "textures/blocks/test.png"
        )
        ...
        self.assertEqual(
            mod_config._to_java_constant("123test"), "_123TEST"
        )  # Starts with digit
        self.assertEqual(
            mod_config._to_java_constant("test-item.name"), "TEST_ITEM_NAME"
        )  # Special chars
        self.assertEqual(
            mod_config._to_java_constant("ALREADY_UPPER"), "ALREADY_UPPER"
        )  # Already uppercase
    ...
    def test_fabricpy_module_imports(self):
        ...
        food = fabricpy.FoodItem(
            id="import_test:food", name="Import Food", nutrition=1, saturation=0.1
        )
        ...
        fabricpy.RecipeJson(
            {
                ...
                "pattern": ["#"],
                "key": {"#": "minecraft:stone"},
                "result": {"id": "import_test:item", "count": 1},
            }
        )

        # Test vanilla group access
        self.assertEqual(fabricpy.item_group.TOOLS, "tools_and_utilities")
        self.assertEqual(fabricpy.item_group.FOOD_AND_DRINK, "food_and_drinks")
