...
class TestItemGroupConstants(unittest.TestCase):
    ...
    def test_all_constants_exist(self):
        ...
        for constant in expected_constants:
            self.assertTrue(
                hasattr(item_group, constant), f"Missing constant: {constant}"
            )

    def test_constant_values(self):
        """Test that constants have the expected string values."""
        self.assertEqual(item_group.BUILDING_BLOCKS, "building_blocks")
        self.assertEqual(item_group.NATURAL, "natural_blocks")
        self.assertEqual(item_group.FUNCTIONAL, "functional_blocks")
        self.assertEqual(item_group.REDSTONE, "redstone_blocks")
        self.assertEqual(item_group.TOOLS, "tools_and_utilities")
        self.assertEqual(item_group.COMBAT, "combat")
        self.assertEqual(item_group.FOOD_AND_DRINK, "food_and_drinks")
    ...
    def test_constants_are_strings(self):
        ...
        for constant in constants:
            self.assertIsInstance(constant, str, f"Constant {constant} is not a string")

    def test_constants_are_lowercase(self):
        """Test that all constants follow lowercase naming convention matching Minecraft identifiers."""
        constants = [
            item_group.BUILDING_BLOCKS,
            item_group.NATURAL,
            item_group.FUNCTIONAL,
        ...
        for constant in constants:
            self.assertEqual(
                constant, constant.lower(), f"Constant {constant} is not lowercase"
            )
    ...
    def test_no_spaces_in_constants(self):
        ...
        for constant in constants:
            self.assertNotIn(" ", constant, f"Constant {constant} contains spaces")
